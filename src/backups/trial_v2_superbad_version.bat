...
IF NOT EXIST "%base_project_directory%\" (
    mkdir "%base_project_directory%"
    if errorlevel 1 (
        echo [ERROR] Failed to create directory %base_project_directory% - check permissions (may need to run as administrator).
        pause
        exit /b 1
    )
)
...
if "!token!"=="" (
    ...
    if errorlevel 1 (
        echo [ERROR] Failed to create/update token file at %GITHUB_PAT% - check permissions or directory (try running as administrator).
        pause
        exit /b 1
    )
