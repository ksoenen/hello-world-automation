...
REM Set .gitattributes to handle line endings
echo Setting .gitattributes for consistent line endings...
echo * text=auto > .gitattributes

REM Clean up stray files (potential debug artifacts) before staging
echo Cleaning up stray files...
del /q "still set to: !GITHUB_PAT!*" 2>nul
del /q "till exists!*" 2>nul
if errorlevel 1 (
    echo [DEBUG] No stray files found or cleanup skipped.
) else (
    echo [DEBUG] Stray files cleaned up.
)
