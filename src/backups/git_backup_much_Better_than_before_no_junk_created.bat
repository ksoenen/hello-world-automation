...
REM Cleanup stray files at the start to handle leftovers
echo Cleaning up stray files at start...
del /q *still set to* 2>nul
del /q *till exists* 2>nul
if errorlevel 1 (
    echo [DEBUG] No stray files found at start or cleanup skipped.
) else (
    echo [DEBUG] Stray files cleaned up at start.
)
...
REM Read token from file at GITHUB_PAT path into temporary variable
...
REM Embed token in remote URL to avoid prompts
echo Setting remote with embedded token...
git remote remove origin 2>nul
git remote add origin https://ksoenen:!token!@github.com/ksoenen/hello-world-automation.git
REM Debug: Print current remote URL (redacted token)
echo Current remote URL: https://ksoenen:REDACTED@github.com/ksoenen/hello-world-automation.git

REM Check if repo exists, create if not (using token)
echo Checking if repo exists...
for /f %%i in ('curl -s -o nul -w "%%{http_code}" -H "Authorization: Bearer !token!" https://api.github.com/repos/ksoenen/hello-world-automation') do set "status=%%i"
if "!status!"=="404" (
    echo Repo not found - creating it...
    curl -L -X POST -H "Accept: application/vnd.github+json" -H "Authorization: Bearer !token!" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/user/repos -d "{\"name\":\"hello-world-automation\",\"private\":false}"
    if errorlevel 1 (
...
REM Update .gitignore to ignore stray files
echo Ignoring stray files in .gitignore...
echo 'still set to: !GITHUB_PAT!' >> .gitignore
echo 'till exists!' >> .gitignore

REM Stage changes with debug to track file creation
echo Staging changes...
echo [DEBUG] Directory before add:
dir /b
git add .
echo [DEBUG] Directory after add:
dir /b
